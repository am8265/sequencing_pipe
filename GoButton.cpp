...
if(test) {
    ...
    sendmail("igm-bioinfo@columbia.edu","igm-bioinfo@columbia.edu","Release summary",message.data());
    cout << "SUMMARY\n" << message << "\n\n";
    ...
    void daily(int, char **) {
        ...
        for (int i = 0 ; i<(int)dirs.size(); ++i ) {
            ...
            if(runs.count(run_info[3].substr(1,9))) {
                ...
                sendmail("igm-bioinfo@columbia.edu","igm-bioinfo@columbia.edu",(dirs[i].substr(0,dirs[i].length()-8) + " appears to be an aborted run.").data(),"Please check and if so remove it.\n");
                continue;
            }
        ...
        for (std::map<std::string,RUNINFO>::iterator it=runs.begin(); it!=runs.end(); it++) {
            ...
            if(it->second.run_started) {
                ...
                if(it->second.lims_fcid<=0) {
                    ...
                    sendmail("igm-bioinfo@columbia.edu","igm-bioinfo@columbia.edu",up.data(),update);
                    cout << "message " << up << "\n";
                ...
                if(it->second.lims_rundir=="NULL") {
                    ...
                    if(it->second.run_dir!=it->second.lims_rundir) {
                        sendmail("igm-bioinfo@columbia.edu","igm-bioinfo@columbia.edu",(it->first + " has a rundir mismatch!").data(),"Please investigate.\n");
        ...
        if(opts::email) sendmail("jb4393@cumc.columbia.edu,ag2239@cumc.columbia.edu,igm-bioinfo@columbia.edu","igm-bioinfo@columbia.edu","NovaSeq occupancy report",body.str().data(),true);
...
void load(int argc, char **argv) {
    ...
    if(1) { // while(1) {
        ...
        if(argc==1){
            ...
            " and dsm.is_merged = 30 and (qc.PercentContamination < 0.08 or qc.DragenContamination < 0.05)"
